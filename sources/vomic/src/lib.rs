...
const BASE_URL: &[u8] = b"https://www.vomic.com";
...
const SOURCE_INFO: SourceInfo = SourceInfo {
    ...
    language: "zh",
    author: "Koma",
    description: "Vomic HTML scraping source.",
    content_rating: "safe",
};
...
fn desc_from_i32(raw: i32) -> HtmlDescriptor {
    unsafe { core::mem::transmute::<i32, HtmlDescriptor>(raw) }
}
...
fn build_get_request(dst: &mut [u8], url: &[u8], referer: Option<&[u8]>) -> Option<usize> {
    ...
    write_bytes(dst, &mut c, br#"},"timeoutMs":15000,"responseKind":"bodyText"}"#)
        .then_some(())?;
    Some(c)
}
...
fn fetch_html(url: &[u8], referer: Option<&[u8]>) -> Result<usize, FetchError> {
    ...
    let req = unsafe { core::slice::from_raw_parts(core::ptr::addr_of!(HTTP_REQ_BUF) as *const u8, req_len) };
    let len = http_request(req, http_out()).map_err(|_| FetchError::Network)?;
    let resp = unsafe { core::slice::from_raw_parts(core::ptr::addr_of!(HTTP_OUT) as *const u8, len) };
    decode_json_body(resp)
}

fn make_absolute_url<'a>(url: &[u8], out: &'a mut [u8]) -> Option<&'a [u8]> {
...
fn normalize_chapter_path<'a>(href: &[u8], out: &'a mut [u8]) -> Option<&'a [u8]> {
    let start = if href.starts_with(BASE_URL) {
        BASE_URL.len()
    } else if href.starts_with(b"http://www.vomic.com") {
        b"http://www.vomic.com".len()
    } else {
        0
    };
    let mut end = start;
...
fn write_manga_item(
    ...
    ok && write_bytes(
        payload,
        c,
        br#","authors":[],"status":"unknown","contentRating":"safe","description":"","sourceTags":["vomic"]}"#,
    )
}
...
fn run_search(req: &[u8]) -> u32 {
    ...
    if !(write_bytes(url_buf, &mut uc, BASE_URL)
        && write_bytes(url_buf, &mut uc, b"/search?keyword=")
    ...
    let url = unsafe { core::slice::from_raw_parts(core::ptr::addr_of!(SCRATCH_A) as *const u8, uc) };
    let html_len = match fetch_html(url, Some(BASE_URL)) {
        Ok(n) => n,
        Err(e) => {
    ...
    let html = unsafe { core::slice::from_raw_parts(core::ptr::addr_of!(BODY_BUF) as *const u8, html_len) };
    ...
    let count = html_select_all(doc.0.raw(), b"a[href*=\"/comic/\"]", selects);
    ...
    for i in 0..max {
        ...
        let raw = i32::from_le_bytes([selects[off], selects[off + 1], selects[off + 2], selects[off + 3]]);
        if raw <= 0 {
            continue;
        }
        ...
        if let (Some(id), Some(title_len)) = (id, title_len) {
            ...
            if !title.is_empty() {
                ...
                let cover_abs = cover_len.and_then(|n| make_absolute_url(trim_ascii(&cover_buf[..n]), &mut cover_abs));
...
fn fetch_detail_doc(id: &[u8], op: &str) -> Result<(usize, OwnedDescriptor), u32> {
    let url_buf = scratch_a();
    let mut uc = 0usize;
    if !(write_bytes(url_buf, &mut uc, BASE_URL)
    ...
    let url = unsafe { core::slice::from_raw_parts(core::ptr::addr_of!(SCRATCH_A) as *const u8, uc) };
    let html_len = fetch_html(url, Some(BASE_URL)).map_err(|e| {
        let (c, m) = fetch_error_code(e);
        write_error(op, c, m)
    })?;
    let html = unsafe { core::slice::from_raw_parts(core::ptr::addr_of!(BODY_BUF) as *const u8, html_len) };
    let doc = html_parse(html).map_err(|_| write_error(op, "parse_error", "html_parse failed"))?;
    Ok((html_len, OwnedDescriptor(doc)))
}
...
fn first_attr(doc: HtmlDescriptor, selectors: &[&[u8]], attr: &[u8], out: &mut [u8]) -> Option<usize> {
    let mut i = 0usize;
    while i < selectors.len() {
        if let Ok(d) = html_select(doc, selectors[i]) {
...
fn run_get_manga(req: &[u8]) -> u32 {
    ...
    let (_html_len, doc) = match fetch_detail_doc(id, "get_manga") {
        Ok(v) => v,
        Err(ptr) => return ptr,
    };

    let mut title_buf = [0u8; 512];
    let title_len = first_text(doc.0, &[b"h1", b".comic-title", b".detail-title", b".title"], &mut title_buf)
    ...
    let cover_len = first_attr(
        doc.0,
        &[b".cover img", b".comic-cover img", b".detail-cover img", b"img"],
        b"data-src",
        &mut cover_buf,
    )
    .or_else(|| {
    ...
    let author_len = first_text(doc.0, &[b".author", b".authors", b"[class*=\"author\"]"], &mut author_buf);
    let mut desc_buf = [0u8; 2048];
    let desc_len = first_text(
        doc.0,
        &[b".description", b".summary", b".intro", b"[class*=\"desc\"]", b"[class*=\"intro\"]"],
        &mut desc_buf,
    );
    ...
    let mut c = 0usize;
    let mut cover_abs = [0u8; 768];
    let cover = cover_len.and_then(|n| make_absolute_url(&cover_buf[..n], &mut cover_abs));
    let ok = write_bytes(payload, &mut c, br#"{"id":"manga:"#)
        ...
        && write_bytes(payload, &mut c, br#"],"status":"unknown","contentRating":"safe","description":""#)
        && if let Some(n) = desc_len {
            append_json_escaped(payload, &mut c, trim_ascii(&desc_buf[..n]))
        } else {
            true
        }
        && write_bytes(payload, &mut c, br#"","sourceTags":["vomic"]}"#);
    if !ok {
...
fn run_get_chapters(req: &[u8]) -> u32 {
    ...
    let (_html_len, doc) = match fetch_detail_doc(id, "get_chapters") {
        Ok(v) => v,
        Err(ptr) => return ptr,
    };

    let selects = select_buf();
    let count = html_select_all(doc.0.raw(), b"a", selects);
    ...
    for i in 0..max {
        ...
        let href = attr_into(hd, b"href", &mut href_buf);
        let path = href.and_then(|h| normalize_chapter_path(h, &mut path_buf));
        let is_chapter = path
...
fn write_page_item(payload: &mut [u8], c: &mut usize, chapter_id: &[u8], idx: usize, url: &[u8]) -> bool {
...
fn append_js_image_urls(html: &[u8], payload: &mut [u8], c: &mut usize, chapter_id: &[u8], written: &mut usize) -> bool {
    let mut pos = 0usize;
    while pos < html.len() && *written < 1000 {
        ...
        if is_image_url(url) {
            ...
            if !write_page_item(payload, c, chapter_id, *written, url) {
                return false;
            }
            *written += 1;
        }
...
fn run_get_pages(req: &[u8]) -> u32 {
    ...
    let url_buf = scratch_a();
    let url = match make_absolute_url(path, url_buf) {
        Some(v) => v,
    ...
    let selects = select_buf();
    let count = html_select_all(doc.0.raw(), b"img", selects);
    let max = if count > 0 { count as usize } else { 0 }.min(1000);
    let mut written = 0usize;
    for i in 0..max {
        let off = i * 4;
        ...
        let src_len = html_attr(hd, b"data-src", &mut src_buf)
            .ok()
            .filter(|n| *n > 0)
            .or_else(|| html_attr(hd, b"data-original", &mut src_buf).ok().filter(|n| *n > 0))
        ...
        if let Some(src_len) = src_len {
            ...
            if is_image_url(src) {
                ...
                if let Some(abs) = make_absolute_url(src, &mut abs_buf) {
                    if written > 0 && !write_bytes(payload, &mut c, b",") {
                        return write_error("get_pages", "internal_error", "payload overflow");
                    }
    ...
    if written == 0 && !append_js_image_urls(html, payload, &mut c, chapter_id, &mut written) {
        return write_error("get_pages", "internal_error", "payload overflow");
    }
...
fn run_get_image_request(req: &[u8]) -> u32 {
    ...
    let ok = write_bytes(payload, &mut c, br#"{"url":""#)
        && append_json_escaped(payload, &mut c, url)
        && write_bytes(payload, &mut c, br#"","headers":{"Referer":"https://www.vomic.com/","User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:121.0) Gecko/20100101 Firefox/121.0"}}"#);
    if !ok {
...
pub extern "C" fn koma_source_get_image_request(req_ptr: u32, req_len: u32) -> u32 {
    ...
    log_info(b"vomic get_image_request");
