...
fn build_get_request(dst: &mut [u8], url: &[u8]) -> Option<usize> {
    ...
    write_bytes(dst, &mut cursor, br#"","headers":{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Safari/537.36"},"timeoutMs":15000,"responseKind":"bodyText"}"#).then_some(())?;
    Some(cursor)
}
...
fn decode_json_body(resp: &[u8]) -> Result<usize, FetchError> {
    ...
    while i < resp.len() {
        ...
        if b == b'\\' && i + 1 < resp.len() {
            ...
            match next {
                b'"' => {
                    if out >= dst.len() { return Err(FetchError::Network); }
                    dst[out] = b'"';
                    out += 1;
                    i += 2;
                }
                ...
                b'n' | b'r' | b't' => {
                    if out >= dst.len() { return Err(FetchError::Network); }
                    dst[out] = if next == b'n' { b'\n' } else if next == b'r' { b'\r' } else { b'\t' };
                    out += 1;
                    i += 2;
                }
                b'u' => {
